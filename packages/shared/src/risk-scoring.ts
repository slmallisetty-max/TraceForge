...
function classifyRiskWithThresholds(
  ...
  if (factors.policy_violations.length > 0) {
    severity = Math.max(
      severity,
      8 + Math.min(factors.policy_violations.length, 2)
    );
  }
...
function generateExplanation(
  ...
  if (factors.policy_violations.length > 0) {
    parts.push(
      `Policy violations detected: ${factors.policy_violations.join(", ")}`
    );
  }

  if (factors.semantic_similarity < 0.7) {
    parts.push(
      `Semantic similarity: ${(factors.semantic_similarity * 100).toFixed(0)}%`
    );
  }
  ...
  if (factors.tone_shift > 0.3) {
    const direction =
      factors.tone_shift > 0 ? "more positive" : "more negative";
  ...
  if (factors.latency_delta && Math.abs(factors.latency_delta) > 0.5) {
    ...
    parts.push(
      `Latency ${(Math.abs(factors.latency_delta) * 100).toFixed(
        0
      )}% ${direction}`
    );
  }
...
export async function calculateRiskScore(
  ...
  if (policyEvaluationResult && policyEvaluationResult.violations) {
    ...
    allPolicyViolations.push(
      ...policyEvaluationResult.violations.map(
        (v: any) =>
          `${severityMapping[v.severity as keyof typeof severityMapping]}: ${
            v.ruleId
          }`
      )
    );
  ...
  const baselineText =
    typeof baselineResponse === "string"
      ? baselineResponse
  ...
  if (!baselineText || !currentText) {
    const latency_delta =
      baseMeta.duration_ms > 0 && currMeta.duration_ms > 0
        ? calculateDelta(baseMeta.duration_ms, currMeta.duration_ms)
        : undefined;
    const token_delta =
      baseMeta.tokens_used > 0 && currMeta.tokens_used > 0
    ...
    return {
      ...
      explanation: !baselineText
        ? "Baseline response is empty"
        : "Current response is empty",
      confidence: 1.0,
    };
  }
  ...
  const length_delta =
    (currentText.length - baselineText.length) / baselineText.length;
  ...
  if (
    ...
    try {
      ...
      console.warn(
        "Failed to calculate semantic similarity, using word overlap:",
        error
      );
...
export function formatRiskScore(score: RiskScore): string {
  ...
  let output = "";
  output += `${
    categoryEmoji[score.category]
  } Risk: ${score.category.toUpperCase()}\n`;
  output += `   Severity: ${score.severity}/10\n`;
  output += `   ${
    recommendationEmoji[score.recommendation]
  ...
  output += `\nRisk Factors:\n`;
  output += `   length_delta: ${(score.factors.length_delta * 100).toFixed(
    1
  )}%\n`;
  output += `   semantic_similarity: ${(
    score.factors.semantic_similarity * 100
  ).toFixed(1)}%\n`;
  output += `   word_overlap: ${(score.factors.word_overlap * 100).toFixed(
    1
  )}%\n`;
  ...
  if (score.factors.latency_delta !== undefined) {
    output += `   latency_delta: ${(score.factors.latency_delta * 100).toFixed(
      1
    )}%\n`;
  }
  if (score.factors.token_delta !== undefined) {
    output += `   token_delta: ${(score.factors.token_delta * 100).toFixed(
      1
    )}%\n`;
  }
...
export function calculateCICDRiskScore(
  ...
  const criticScore = Math.round(
    categoryScores[critic.category] * critic.confidence
  );
...
export function formatCICDRiskScore(score: CICDRiskScore): string {
  ...
  return `${emoji[score.level]} Risk: ${
    score.overall
  }/100 (${score.level.toUpperCase()})`;
}
