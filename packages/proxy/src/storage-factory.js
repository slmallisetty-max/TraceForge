 ...
 */
export function createStorageBackend(type) {
    const storageType = type || process.env.TRACEFORGE_STORAGE_BACKEND || "file";
    switch (storageType) {
        case "sqlite":
 ...
 */
export function createStorageWithFallback() {
    ...
    const primaryType = process.env.TRACEFORGE_STORAGE_BACKEND || "file";
    const fallbackType = primaryType === "sqlite" ? "file" : "sqlite";
    const primary = createStorageBackend(primaryType);
    const fallback = createStorageBackend(fallbackType);
    return new StorageManager({
        primary,
        fallbacks: [fallback],
        retryAttempts: parseInt(process.env.TRACEFORGE_STORAGE_RETRY_ATTEMPTS || "3"),
