  ...
  async generateEmbeddings(texts: string[]): Promise<number[][]> {
    ...
    if (validTexts.length < texts.length) {
      console.warn(
        ...
        } empty text(s) filtered out from embedding request`
      );
    }
    ...
    try {
      ...
      if (error.code === "ECONNREFUSED") {
        throw new Error(
          `Cannot connect to OpenAI API at ${this.baseUrl}. Check network connection.`
        );
      } else if (error.code === "ENOTFOUND") {
        throw new Error(
          `Cannot resolve OpenAI API host. Check base URL: ${this.baseUrl}`
        );
      }
      throw error;
    }
...
export function getDefaultEmbeddingService(
  ...
  if (!apiKey) {
    throw new Error(
      "OPENAI_API_KEY environment variable not set. Required for semantic assertions."
    );
  }
