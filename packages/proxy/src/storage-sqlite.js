...
export class SQLiteStorageBackend {
    ...
    runMigrations() {
      ...
      `);
        ...
        const tables = this.db
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='traces_fts'")
            .all();
        if (tables.length === 0) {
    ...
    async saveTrace(trace) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(redacted.id, redacted.timestamp, redacted.endpoint, JSON.stringify(redacted.request), redacted.response ? JSON.stringify(redacted.response) : null, JSON.stringify(redacted.metadata), redacted.schema_version || "1.0.0", redacted.session_id || null, redacted.step_index !== undefined ? redacted.step_index : null, redacted.parent_trace_id || null, redacted.state_snapshot ? JSON.stringify(redacted.state_snapshot) : null);
    }
    async getTrace(id) {
    ...
    async listTraces(options = {}) {
        const { limit = 100, offset = 0, sortBy = "timestamp", sortOrder = "desc", filter = {}, } = options;
    ...
    async saveTest(test) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(test.id, test.name, test.description || null, test.trace_id || null, JSON.stringify(test.request), JSON.stringify(test.assertions), test.created_at, test.fixtures ? JSON.stringify(test.fixtures) : null, test.tags ? JSON.stringify(test.tags) : null, test.timeout || null, test.policy_contracts ? JSON.stringify(test.policy_contracts) : null);
    }
    async getTest(id) {
