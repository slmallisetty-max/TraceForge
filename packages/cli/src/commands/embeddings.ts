...
export const embeddingsCommand = new Command('embeddings')
  .description('Manage embedding models for semantic assertions');
  ...
  .action(async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(chalk.yellow('Ollama not found. Please install from: https://ollama.ai'));
        console.log(chalk.gray('\nInstallation:'));
        console.log(chalk.white('  macOS/Linux: curl https://ollama.ai/install.sh | sh'));
        console.log(chalk.white('  Windows: Download from https://ollama.ai/download'));
        return;
      }
      ...
      console.log(chalk.cyan('  export TRACEFORGE_EMBEDDING_MODEL=nomic-embed-text'));
...
embeddingsCommand
  ...
  .action(async (options) => {
    try {
      const { getDefaultEmbeddingService, cosineSimilarity } = await import('@traceforge/shared');
      ...
      console.log(chalk.gray(`  Provider: ${service.getProvider()}`));
      console.log(chalk.bold(`  Similarity: ${(similarity * 100).toFixed(2)}%`));
  ...
  .action(async () => {
    try {
      ...
      if (config.provider === 'ollama') {
        console.log(chalk.gray('Base URL:'), chalk.white(config.baseUrl || 'http://localhost:11434'));
      }
      
      console.log(chalk.gray('Cache Enabled:'), chalk.white(config.cacheEnabled ? 'Yes' : 'No'));
      
      if (config.cacheEnabled) {
        console.log(chalk.gray('Cache Path:'), chalk.white(config.cachePath || '.ai-tests/embeddings-cache'));
      }
