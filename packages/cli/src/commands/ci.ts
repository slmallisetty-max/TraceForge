...
ciCommand
  ...
  .option(
    "-b, --baseline <path>",
    "Baseline traces directory",
    ".ai-tests/baseline"
  )
  .option(
    "-c, --current <path>",
    "Current traces directory",
    ".ai-tests/traces"
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const tracePairs = await findTracePairs(
        options.baseline,
        options.current
      );
      ...
      console.log(
        chalk.gray(`Found ${tracePairs.length} test cases to analyze\n`)
      );
      ...
      for (let i = 0; i < tracePairs.length; i++) {
        ...
        process.stdout.write(
          chalk.gray(`[${i + 1}/${tracePairs.length}] Analyzing ${name}... `)
        );
...
async function findTracePairs(
  ...
  try {
    ...
    console.error(
      chalk.red(`Error reading baseline directory: ${baselinePath}`)
    );
    return [];
  }
  ...
  for (const baselineFile of baselineFiles) {
    ...
    try {
      ...
      const baselineContent = await readFile(
        join(baselinePath, baselineFile),
        "utf-8"
      );
      const currentContent = await readFile(
        join(currentPath, matchingCurrent),
        "utf-8"
      );
      ...
      console.warn(
        chalk.yellow(`Warning: Failed to load trace pair ${baselineFile}`)
      );
    }
  }
